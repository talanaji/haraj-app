...
class BlogProvider extends PsProvider {
  ...
  Future<dynamic> loadBlogList(String itemId) async {
    isLoading = true;

    isConnectedToInternet = await Utils.checkInternetConnectivity();
    await _repo.getAllBlogList(blogListStream, isConnectedToInternet, limit,
        offset,itemId, PsStatus.PROGRESS_LOADING);
  }

  Future<dynamic> nextBlogList(String itemId) async {
    isConnectedToInternet = await Utils.checkInternetConnectivity();

    if (!isLoading && !isReachMaxData) {
      super.isLoading = true;
      await _repo.getNextPageBlogList(blogListStream, isConnectedToInternet,
          limit, offset,itemId, PsStatus.PROGRESS_LOADING);
    }
  }

  Future<void> resetBlogList(String itemId) async {
    isConnectedToInternet = await Utils.checkInternetConnectivity();
    isLoading = true;
